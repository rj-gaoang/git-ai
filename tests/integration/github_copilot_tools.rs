...
/// Regression: some legacy Java projects contain non-UTF-8 bytes. The Copilot
/// bash fallback must still keep explicit changed paths in the checkpoint.
#[test]
fn test_run_in_terminal_non_utf8_existing_file_is_attributed() {
    let repo = TestRepo::new();

    let path = repo.path().join("src/test/java/LegacyEncodingTest.java");
    std::fs::create_dir_all(path.parent().unwrap()).unwrap();
    let mut legacy_content = b"class LegacyEncodingTest {\n".to_vec();
    legacy_content.extend_from_slice(b"    // legacy gbk-ish bytes: ");
    legacy_content.extend_from_slice(&[0xE5, 0xA4, b'\n']);
    legacy_content.extend_from_slice(b"}\n");
    std::fs::write(&path, &legacy_content).unwrap();
    repo.git(&["add", "src/test/java/LegacyEncodingTest.java"])
        .unwrap();
    repo.git(&["commit", "-m", "Initial legacy file"]).unwrap();

    let mut updated = legacy_content;
    updated.splice(
        updated.len() - 2..updated.len() - 2,
    ...
    repo.git(&["add", "src/test/java/LegacyEncodingTest.java"])
        .unwrap();
    repo.git(&["commit", "-m", "Append generated test to legacy file"])
        .unwrap();
    repo.sync_daemon();
