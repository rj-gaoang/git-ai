...
每条 JSONL 日志都会带 `timestampMs` 和 UTC `timestamp`。当 `debug.jsonl` 超过 2GB 时，`git-ai` 会尽力保留最近约 512MB 内容后继续写入；日志创建、写入或裁剪失败都静默忽略，不允许影响 commit、stats 或上传主流程。
