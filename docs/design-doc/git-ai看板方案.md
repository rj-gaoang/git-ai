...
> **实施补充（2026-05-04，同一 commit 支持多段 prompt segment）**：在继续下钻“一个 commit 可能由多段提示词共同驱动”的场景后，当前实现已不再把同一 AI session 的多次 checkpoint 统一压成一个 session 级 prompt hash。具体做法是：AI checkpoint 写 working log 时新增显式 `prompt_id`，按“本次 checkpoint / 本次 prompt segment”生成稳定短 hash，并把本次新增行直接归到这个 segment hash；`virtual_attribution.rs` 聚合 working log 时，也改为优先使用 `checkpoint.prompt_id`，同时只保留“相对上一条同 session checkpoint 新增的用户消息”作为当前 `PromptRecord.messages`。这样一次 commit 里即使来自同一个 session，也能同时保留多段 prompt；而完全手动生成代码、没有 AI checkpoint 的提交，则仍然自然表现为 `0` 条 prompt。为兼容旧 note / 旧 working log，缺失 `prompt_id` 的历史数据仍回退到旧 session hash 读取。

> **实施补充（2026-05-04，checkpoint transcript 快照保留）**：在 `op-return-exchange` 最近一次 commit 的 `debug.jsonl` 排查中又确认了一条独立失败路径：虽然上一步已经把同一 session 拆成多段 prompt segment，但 `RepoStorage::append_checkpoint()` 仍会对带 `chat_session_path` / `session_path` 等 refetch 元数据的 AI checkpoint 清空 inline transcript，而 `post_commit::update_prompts_to_latest()` 只会为同一会话的最后一个 checkpoint 回填最新 transcript。结果就是最后一个 checkpoint 仍会重新吃进整段当前聊天会话，把更早、与这次 commit 无关的用户输入一并带到本次上传里。当前实现已改为保留每个 AI checkpoint 当时的 transcript 快照，refetch 只作为补充更新来源，不再依赖“丢掉历史 transcript 再只回填最后一条”。新增 `test_refetchable_ai_checkpoints_keep_inline_transcript_snapshots` 回归测试覆盖 `pi` 和 `github-copilot` 场景。
...
4. async daemon 路径由后台服务执行上传；sync wrapper 路径以内联方式完成上传，避免宿主进程过快退出导致上传线程被带死。HTTP timeout 为 10 秒，上传失败不影响 commit 成功。
5. async daemon 启动阶段如果遇到 `daemon.lock` 被占用但 control / trace socket 不可用，`git-ai` 会先等待同伴进程完成启动；若超时且能从 `daemon.pid.json` 读到 pid，会自动强制回收旧 daemon 并拉起新 daemon。若 Windows 拒绝结束旧进程（例如旧 daemon 由更高权限进程拉起），新版会激活一套 replacement daemon runtime / pipe，并直接更新全局 Git 配置文件里的 trace2 指向，避免继续被旧锁和旧 pipe 困住。
...
| 1.5 | （可选）按本文「需求 1 步骤 3」修改 check-prerequisites.ps1 | `.specify/scripts/powershell/check-prerequisites.ps1` | 存量旧项目中也能提示 git-ai 缺失 |
...
| 2.10 | 修复 GitHub Copilot native hook transcript fallback 的 VS Code 后缀兼容 | `git-ai/src/commands/checkpoint_agent/agent_presets.rs` | `tool_input="..."` 且 hook `tool_use_id=call_xxx__vscode-<digits>` 时，仍能命中 transcript 中的 `toolCallId=call_xxx`，生成正确的 `AiAgent` checkpoint |
...
| 2.20 | 同一 AI session 的多次 checkpoint 改为按 prompt segment 记录，单个 commit 可保留多段提示词 | `git-ai/src/authorship/working_log.rs`、`git-ai/src/commands/checkpoint.rs`、`git-ai/src/authorship/virtual_attribution.rs`、`git-ai/src/authorship/authorship_log_serialization.rs`、`git-ai/src/authorship/internal_db.rs`、`git-ai/src/authorship/rebase_authorship.rs`、`git-ai/docs/design-doc/git-ai看板方案.md` | AI checkpoint 新增显式 `prompt_id`，working log / note / 本地 SQLite / rebase roundtrip 都保留 segment hash；同一 session 的后续 checkpoint 只记录新增用户消息，不再把整段历史 transcript 重复挂到最后一条 prompt 上；新增 `test_commit_keeps_multiple_prompt_segments_from_same_session` 回归测试，当前工作区级静态诊断 `get_errors` 无报错，终端 `cargo test` 仍受本机 GNU linker 环境阻塞 |
| 2.21 | 保留 refetchable AI checkpoint 的 transcript 快照，避免同一会话旧 prompt 被最后一条 checkpoint 一并上传 | `git-ai/src/git/repo_storage.rs`、`git-ai/docs/design-doc/git-ai看板方案.md` | `append_checkpoint()` 不再因为 `chat_session_path` / `session_path` 等 refetch 元数据清空 inline transcript；同一 session 多次 AI checkpoint 提交时，旧 prompt 不会再被最后一个 checkpoint 吞并进本次 commit；`cargo test git::repo_storage::tests::test_refetchable_ai_checkpoints_keep_inline_transcript_snapshots -- --exact`、`cargo test authorship::virtual_attribution::tests::test_commit_keeps_multiple_prompt_segments_from_same_session -- --exact`、`cargo test commands::checkpoint::tests::test_ai_checkpoint_reclaims_recent_known_human_file_capture -- --exact` 与 `cargo test commands::checkpoint::tests::test_ai_follow_up_checkpoint_preserves_previous_ai_ranges -- --exact` 均通过 |
...
- 如果 `rj-gaoang/git-ai` 的 latest release 变了，Spec Kit 下次 force-update 时拉到的就会是新的 latest release 二进制。
...
### 2026-05-04：同一 commit 支持多段 prompt segment

**变更原因：**

前一轮“prompt 仅保留用户输入”解决的是 assistant transcript 泄漏问题，但并没有解决另一个更本质的建模问题：旧实现始终把同一 AI session 的多次 checkpoint 压成同一个 session hash，并用最后一次 checkpoint 的整段 transcript 覆盖前面的 prompt 记录。结果就是一次 commit 即使实际上由多段提示词共同驱动，note / SQLite / 上传 payload 最终也只能看到“同一 session 的最后一大段 prompt”，既不能正确表达“本次提交有多个提示词”，也会把前面已经失效的上下文重复带到后面那条 prompt 上。

**本次代码级修改：**

| 文件 | 修改点 | 影响 |
|------|--------|------|
| `git-ai/src/authorship/working_log.rs` | 给 AI checkpoint 新增可选字段 `prompt_id` | working log 现在能显式保存“本次 checkpoint / 本次 prompt segment”的 hash，而不是只能从 `agent_id` 反推 session hash |
| `git-ai/src/commands/checkpoint.rs` | AI checkpoint 写入时改为生成 per-checkpoint `prompt_id`，并把新增行直接归到这个 segment hash；metrics attrs 也优先带 `prompt_id` | 同一 session 的两次 AI 编辑不再落到同一个 prompt hash，下游 accepted / overridden 统计也能继续沿用现有 line attribution 口径 |
| `git-ai/src/authorship/virtual_attribution.rs` | 聚合 checkpoint prompt 时优先使用 `checkpoint.prompt_id`；同一 session 的 transcript 只保留“相对上一条 checkpoint 新增的 `Message::User`” | 一次 commit 可以同时保留多段 prompt，且后一个 checkpoint 不会再把前一个 prompt 文本重复带进自己的 `PromptRecord.messages` |
| `git-ai/src/authorship/authorship_log_serialization.rs` | note -> working log roundtrip 时不再重新用 `agent_id` 生成 session hash，而是保留 attestation 已引用的 `prompt_id` | amend / rebase / 重新序列化 note 时，不会把 prompt segment 再压回旧的 session hash |
| `git-ai/src/authorship/internal_db.rs` | 本地 SQLite `PromptDbRecord::from_checkpoint(...)` 优先写入 `checkpoint.prompt_id` | `prompt_storage=local/default` 时，本地 prompt 记录与 note / 上传的 segment 语义保持一致 |
| `git-ai/src/authorship/rebase_authorship.rs` | 由 checkpoint 生成临时 authorship note 时，prompt metadata 优先使用 `checkpoint.prompt_id` | rebase 过程中同一 session 的多段 prompt 不会因为中间 note 重建而重新合并 |
| `git-ai/src/authorship/virtual_attribution.rs` | 新增 `test_commit_keeps_multiple_prompt_segments_from_same_session` | 用同一 session 的两次 AI checkpoint 覆盖“一次 commit 两段 prompt”场景，锁定这次回归面 |

**验证结论：**

- 工作区级 `get_errors` 静态诊断对本次涉及的 `checkpoint.rs`、`virtual_attribution.rs`、`authorship_log_serialization.rs`、`internal_db.rs`、`rebase_authorship.rs`、`working_log.rs` 均无报错。
- 已新增聚焦回归测试 `test_commit_keeps_multiple_prompt_segments_from_same_session`，覆盖“同一 session、两次 AI checkpoint、一个最终 commit”的场景。
- 当前终端里的 Rust host 仍先命中本机 GNU 链接链路，`cargo test test_commit_keeps_multiple_prompt_segments_from_same_session --lib` 尚被 `_Unwind_Resume` / `_GCC_specific_handler` 链接错误阻塞；这属于已知 Windows 工具链环境问题，不是本次代码改动本身的新编译错误。

### 2026-05-04：保留 AI checkpoint transcript 快照，避免同一会话旧 prompt 混入当前 commit

**变更原因：**

上一节把同一 session 的多次 AI checkpoint 拆成了多个 prompt segment，但 `op-return-exchange` 最近一次 commit 的 `debug.jsonl` 仍暴露出一个存储层残留问题：`RepoStorage::append_checkpoint()` 过去会对“可 refetch transcript 的工具”直接清空 `checkpoint.transcript`，而 `post_commit::update_prompts_to_latest()` 只会为同一 `(tool, session_id)` 的最后一个 checkpoint 回填最新 transcript。这样一来，虽然前面几个 checkpoint 已经有了不同的 `prompt_id`，真正进入 `virtual_attribution` 的却只有最后一个 checkpoint 的整段会话 transcript，结果仍然会把更早、与本次 commit 无关的提示词一起上传。

**本次代码级修改：**

| 文件 | 修改点 | 影响 |
|------|--------|------|
| `git-ai/src/git/repo_storage.rs` | `append_checkpoint()` 改为始终保留 checkpoint 自带的 transcript 快照，不再因为 `chat_session_path` / `session_path` / `transcript_path` 等 refetch 元数据把 AI checkpoint 的 inline transcript 清空；原有 `pi` 测试断言同步改成“保留快照”，并新增 `github-copilot` 覆盖 | working log 现在能真正保存“每个 checkpoint 当时看到的用户提示词边界”，后续 `virtual_attribution` 做 prompt segment 增量切片时，不会再退化成“前面若干 prompt ID 为空，最后一个 prompt ID 吃掉整段当前会话” |
| `git-ai/docs/design-doc/git-ai看板方案.md` | 回填这次 root cause、Phase 2 新增验收项和验证命令 | 后续再排查 prompt 上传范围时，可以明确区分“segment hash 设计”与“checkpoint transcript 快照保留”这两层约束，避免只修了一半又回归 |

**验证结论：**

- `cargo test git::repo_storage::tests::test_refetchable_ai_checkpoints_keep_inline_transcript_snapshots -- --exact` 已通过，覆盖 `pi` 和 `github-copilot` 这两条原本会走 refetch 分支的 checkpoint 存储路径。
- `cargo test authorship::virtual_attribution::tests::test_commit_keeps_multiple_prompt_segments_from_same_session -- --exact` 已通过，说明保留 transcript 快照后，原有“同一 session 多段 prompt segment”归因链路仍然成立，没有被这次存储修复破坏。
- `cargo test commands::checkpoint::tests::test_ai_checkpoint_reclaims_recent_known_human_file_capture -- --exact` 已通过，说明最近几轮为修复代码归因补上的 `KnownHuman` 抢占回收逻辑没有被这次 transcript 存储改动带回去。
- `cargo test commands::checkpoint::tests::test_ai_follow_up_checkpoint_preserves_previous_ai_ranges -- --exact` 已通过，说明 AI follow-up checkpoint 复用旧基线时保留整块历史 AI 归因的逻辑也没有被这次改动破坏。
...
`git_ai_prompt_stats.prompt_text` 是否能稳定落库，直接取决于 note 中是否保留 `prompts[].messages` 里的用户消息。旧默认 `prompt_storage=default` 会倾向 CAS / prompt store，并从 note 中清掉 messages，导致上传端拿不到正文。同时，现场排查归因问题时只靠 stderr 不够稳定，有些用户环境里看不到打印输出，需要本地文件日志兜底。

**本次代码级修改：**

| 文件 | 修改点 | 影响 |
|------|--------|------|
| `git-ai/src/commands/checkpoint_agent/agent_presets.rs` | GitHub Copilot native hook transcript 精确回查在匹配 `tool_use_id` / `toolCallId` 时兼容 VS Code 附加的 `__vscode-<digits>` 后缀，并新增单测覆盖“hook ID 带后缀 + transcript ID 不带后缀”的真实场景 | 修复 VS Code hook `tool_input="..."` 时，明明存在成功 `apply_patch` / `create_file` transcript 记录却仍无法生成 `AiAgent` checkpoint、最终被 `KnownHuman` 归成人工的问题 |
