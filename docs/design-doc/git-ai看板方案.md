...
> **实施补充（2026-05-05，上线代码 AI 占比 Phase 1）**：为区分“开发过程 commit 统计”和“最终上线代码统计”，当前已补第一阶段数据闭环：`git-ai` 新增 `upload-online-stats` / `upload-ai-online-stats` 命令，基于 `baseCommit..headCommit` 的 range authorship 计算最终 diff 留存行，并上传到新的 `/api/public/upload/ai-online-stats`；`ai-cr-manage-service` 新增 `git_ai_online_stats` 总表、上传接口和 `/git-ai/dashboard/online/stats`、`/git-ai/dashboard/online/detail` 查询接口。第一阶段只验证总量：上线总新增行、AI 行、人工行、unknown 行和比例，不做前端、不做成员榜、不做全库快照；数据库建表 SQL 已整理到 `ai-cr-manage-service/docs/git_ai_online_stats.sql`，详细架构见 `docs/design-doc/git-ai上线代码AI占比统计方案.md`。
...
| 2.20 | 增加上线代码范围统计上传命令 | `git-ai/src/commands/git_ai_handlers.rs`、`git-ai/src/integration/upload_stats.rs`、`git-ai/docs/design-doc/git-ai上线代码AI占比统计方案.md` | 执行 `git-ai upload-online-stats <base>..<head> --dry-run` 能基于 range authorship 生成上线统计 payload；执行 `git-ai upload-online-stats --base <base> --head <head>` 能上传到 `/api/public/upload/ai-online-stats` |
...
| 4.6 | 新增上线代码 AI 占比 Phase 1 后端数据闭环 | `ai-cr-manage-service/ruijie-codereview-manage/src/main/java/com/ruoyi/codereview/gitai/*`、`ai-cr-manage-service/docs/git_ai_online_stats.sql` | `POST /api/public/upload/ai-online-stats` 能幂等写入 `git_ai_online_stats`；`GET /git-ai/dashboard/online/stats` 和 `/online/detail` 能查询上线总量和最近明细 |
...
### 2026-05-05：上线代码 AI 占比 Phase 1 数据闭环

**变更原因：**

原有看板主链路统计的是开发过程中每个 commit 的 AI 使用情况，不能直接说明“最终合并到 master / 发布到生产的代码里 AI 占多少”。这次先不做前端和排行榜，只补最小数据闭环，让 CI 能以 `baseCommit..headCommit` 为范围计算最终 diff 留存行，并把总量结果存入后端，方便先核对数据对不对。

**本次代码级修改：**

| 文件 | 修改点 | 影响 |
|------|--------|------|
| `git-ai/src/commands/git_ai_handlers.rs` | 新增 `upload-online-stats` / `upload-ai-online-stats` 命令解析，支持 `--base`、`--head`、`<base>..<head>`、`--dry-run`、项目/分支/环境/MR/CI 上下文字段 | CI 或本地可直接触发上线范围统计上传，不再需要先把统计 JSON 落成文件 |
| `git-ai/src/integration/upload_stats.rs` | 新增 `upload_online_range_stats(...)`、上线统计上传 URL 解析、range authorship payload 构造和 dry-run 摘要 | 基于最终 `base..head` diff 统计 `totalAddLines`、`aiAddLines`、`humanAddLines`、`unknownAddLines`、`aiRate`、`unknownRate`、`noteCoverageRate` |
| `git-ai/docs/design-doc/git-ai上线代码AI占比统计方案.md` | 重排为“整体架构先行”，补充数据流图、时序图、Phase 1 执行顺序和命令示例 | 先讲清楚 commit、`refs/notes/ai`、统计 JSON 三类数据怎么走，避免误解为“用户要把统计结果 push 到 Git 仓库” |
| `ai-cr-manage-service/ruijie-codereview-manage/.../CodereviewApiPublicDataController.java` | 新增 `POST /api/public/upload/ai-online-stats` | 接收 git-ai / CI 上传的上线统计结果 |
| `ai-cr-manage-service/ruijie-codereview-manage/.../gitai/controller/GitAiStatsController.java` | 新增 `/git-ai/dashboard/online/stats` 和 `/git-ai/dashboard/online/detail` | 不做前端也能通过接口核对上线总量和最近明细 |
| `ai-cr-manage-service/ruijie-codereview-manage/.../gitai/domain`、`mapper`、`service` | 新增上线统计 DTO、Entity、VO、Mapper、Service 和 upsert 逻辑 | 以 `(repo_url, environment, metric_scope, base_commit, head_commit)` 做幂等键存储 Phase 1 总量 |
| `ai-cr-manage-service/docs/git_ai_online_stats.sql` | 新增 `git_ai_online_stats` 建表 SQL，并预留成员/文件表 | 数据库由运维/用户先手工建表，当前代码先使用总表验证数据 |

**验证结果：**

- `cargo fmt` 通过。
- `cargo check` 通过；仅保留仓库原有 `line_range_overlap_len` dead code warning。
- `mvn -pl ruijie-codereview-manage -am -DskipTests compile` 通过。
