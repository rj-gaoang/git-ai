...
> **实施补充（2026-05-19，安装 / 更新成功测试数据）**：`install.ps1` 和 `install.sh` 现在会在安装或更新成功后，调用新原生命令 `git-ai upload-install-test --source installTest --quiet` 向远程看板发送一条零行数的合成测试数据。该数据复用 `/api/public/upload/ai-stats`、`GIT_AI_REPORT_REMOTE_*` 和 MCP user-id 解析逻辑，并把当前 `git-ai` 版本号同时放进 commit message、prompt text、prompt `customAttributes.gitAiVersion`、tool model 和 `clientContext.gitAiCliVersion`。上传失败只作为 best-effort warning / debug 事件处理，不会让安装或更新失败；如需跳过，可在运行安装器前设置 `GIT_AI_SKIP_INSTALL_TEST_UPLOAD=1`。
...
| 2.22 | 安装 / 更新成功后发送看板测试数据并携带 git-ai 版本号 | `git-ai/install.ps1`、`git-ai/install.sh`、`git-ai/src/commands/git_ai_handlers.rs`、`git-ai/src/integration/upload_stats.rs` | 执行安装器或 `git-ai upgrade` 完成后，会 best-effort 调用 `git-ai upload-install-test --source installTest --quiet`；payload 为零行数合成记录，并在 commit / prompt / customAttributes / model / clientContext 中携带当前版本；`GIT_AI_SKIP_INSTALL_TEST_UPLOAD=1` 可跳过 |
...
- 开发者：默认每次 commit 成功后立即尝试上传；安装或更新成功后也会发送一条携带 `git-ai` 版本号的零行数看板测试数据；如果需要显式补传当前或指定 commit，可执行 `git-ai upload-stats [<commit>...]` 或别名 `git-ai upload-ai-stats [<commit>...]`；如果需要历史范围回补，仍可执行 `upload-ai-stats.ps1`；如需关闭即时上传，可设置 `GIT_AI_AUTO_UPLOAD_AI_STATS=false`，如需跳过安装测试数据，可设置 `GIT_AI_SKIP_INSTALL_TEST_UPLOAD=1`
...
### 2026-05-19：安装 / 更新成功后发送版本测试数据

**变更原因：**

安装和更新链路过去只能从本机日志或后续真实 commit 判断“新版本是否真的接入看板”。这次把成功安装 / 更新本身也变成一条可观测的测试数据，并要求测试数据必须携带当前 `git-ai` 版本号，方便远程看板确认用户机器已完成升级。

**本次代码级修改：**

| 文件 | 修改点 | 影响 |
|------|--------|------|
| `git-ai/src/integration/upload_stats.rs` | 新增安装测试 payload 构造与上传入口，复用既有 URL/API key/user-id/HTTP 成功判定逻辑 | 不新增服务端接口；合成记录使用零行数，版本号写入 commit message、prompt text、prompt customAttributes、tool model 和 clientContext |
| `git-ai/src/commands/git_ai_handlers.rs` | 新增 `upload-install-test` 子命令，支持 `--dry-run`、`--source`、`--quiet` | 安装器可以在非仓库环境中触发测试上传，调试时也可 dry-run 查看 payload 摘要 |
| `git-ai/install.ps1` | 安装成功、释放 `upload_activity.lock` 后静默调用 `upload-install-test` | 避免 Windows 安装锁与上传锁互相等待；上传失败只警告，不影响安装 / 更新成功 |
| `git-ai/install.sh` | 安装成功后静默调用 `upload-install-test` | Unix/macOS/Linux 新装和 upgrade 复用同一安装器路径，都会发送同一类测试数据 |
| `git-ai/README.md`、`git-ai/docs/design-doc/git-ai环境变量与配置说明.md`、`git-ai/docs/design-doc/git-ai看板方案.md` | 补充安装测试上传行为和 `GIT_AI_SKIP_INSTALL_TEST_UPLOAD=1` | 用户和排查文档与当前实现一致 |

**验证结论：**

- `cargo test --lib integration::upload_stats` 已覆盖安装测试 payload 的版本字段、零行统计、40 位合成 commit code 和 user-id 字段。
- `cargo test --lib commands::git_ai_handlers` 已覆盖 `upload-install-test` 参数默认值、`--dry-run` / `--quiet` / `--source=` 解析和未知参数拒绝。
- 该上传继续校验 HTTP 2xx 与服务端 JSON `code == 200`；安装器侧只把失败当 warning，因此网络或服务端不可用不会阻断安装 / 更新。
