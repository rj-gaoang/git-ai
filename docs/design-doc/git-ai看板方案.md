...
| `git-ai/src/transcripts/model_extraction.rs` | 兼容 Copilot transcript 中 `data.modelId` / `data.modelID` 等大小写差异字段 | 当 IDE transcript 实际携带模型字段时，修复服务端 prompt/model 落库时 `model` 丢失或退回 `unknown` 的情况；若 transcript 本身没有模型字段，当前仍会退回 `unknown` |
