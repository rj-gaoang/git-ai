...
> **当前快速实现决策（2026-04-26）**：如果目标是“尽快让 `git_ai_prompt_stats.prompt_text` 稳定落库”，当前最简单方案就是统一切到 `notes` 模式，而不是继续补 CAS / SQLite 回填链路。原因很直接：`notes` 模式下，`git-ai` 会在脱敏后把 prompt messages 直接保留在 `refs/notes/ai` 中，现有上传脚本和 Rust 原生自动上传都可以直接从 note 里取到 `promptText`，不需要再额外读取 `messages_url`、CAS 或本地 SQLite。
...
**补充配置：prompt 存储模式建议直接切到 `notes`**

如果当前目标是“快速实现提示词正文随现有上传链路一起落库”，建议不要继续使用默认的 `prompt_storage=default`，而是直接切到 `notes`。
...
如果未来要回到更严格的隐私边界，再切回 `default`，并补一条“上传时根据 `messagesUrl` / CAS / 本地 SQLite 回填 prompt 正文”的链路即可。
...
- 开发者：默认每次 commit 成功后立即尝试上传；如果需要历史回补，仍可执行 `upload-ai-stats.ps1`；如需关闭即时上传，可设置 `GIT_AI_AUTO_UPLOAD_AI_STATS=false`
