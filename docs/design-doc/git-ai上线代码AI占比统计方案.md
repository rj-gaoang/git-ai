...
这份方案先只解决一个核心问题：**最后真正进入 master / release / 生产部署的代码里，AI 代码到底占多少。**

第一阶段先做最小数据闭环，不做前端看板，不做成员榜，不做全库快照：

```text
开发者本地生成归因证据 -> notes 自动同步到远端 -> CI 在 base..head 上统一计算 -> 后端入库 -> 用接口查数据
```
...
| 数据 | 存在哪里 | 谁产生 | 用来干什么 |
| --- | --- | --- | --- |
| 代码 commit | Git 远端仓库 | 开发者正常 push | 确定最终上线了哪些代码 |
| 归因证据 `refs/notes/ai` | Git notes 远端引用 | git-ai 在 commit 后生成，push 时同步 | 判断新增行是 AI、人还是 unknown |
| 上线统计结果 JSON | 后端数据库 | CI 统一计算后上传 | 给接口、看板和人工核对使用 |

最容易误解的点是：**用户不需要把本地统计结果 JSON push 到 Git 仓库。**

真正需要同步到远端的是 `refs/notes/ai` 这份归因证据。统计结果由 CI 计算后上传到后端。
...
flowchart LR
    ...
    Commit --> Note[git-ai 写入归因证据<br/>refs/notes/ai]
    Note --> Push[正常 git push]
    Push --> Repo[远端仓库<br/>代码 commit + refs/notes/ai]
    Repo --> CI[GitLab CI<br/>master合并成功或部署成功]
    CI --> Fetch[fetch 代码历史和 refs/notes/ai]
    Fetch --> Stats[git-ai upload-online-stats base..head]
    Stats --> Upload[POST /api/public/upload/ai-online-stats]
    Upload --> DB[(git_ai_online_stats)]
    DB --> API[/git-ai/dashboard/online/stats<br/>/git-ai/dashboard/online/detail]
    API --> Check[先用接口核对数据]
```
...
sequenceDiagram
    ...
    Repo-->>CI: master 合并成功或部署成功后触发
    CI->>Repo: fetch 代码历史和 refs/notes/ai
    CI->>CI: git-ai upload-online-stats base..head
    CI->>Backend: POST /api/public/upload/ai-online-stats
    Backend->>DB: upsert git_ai_online_stats
    Backend-->>CI: 返回上传成功或失败
```
...
| 角色 | 第一阶段要做的事 | 不做的事 |
| --- | --- | --- |
| 开发者 | 正常使用 git-ai commit / push | 不手工上传统计 JSON |
| git-ai | 生成 `refs/notes/ai`，计算 `base..head` 上线统计，上传后端 | 第一阶段不强求成员/文件明细 |
| CI | 选择 `baseCommit/headCommit`，fetch notes，调用 git-ai 上传 | 不把统计结果写回 Git 仓库 |
| 后端 | 接收上线统计、幂等入库、提供查询接口 | 第一阶段不做前端页面 |

### 0.5 Phase 1 已开始落地的代码范围

这次先做数据闭环，代码范围如下：

1. 后端新增上传接口：`POST /api/public/upload/ai-online-stats`。
2. 后端新增查询接口：`GET /git-ai/dashboard/online/stats`。
3. 后端新增查询接口：`GET /git-ai/dashboard/online/detail`。
4. 后端新增总表实体：`git_ai_online_stats`。
5. git-ai 新增命令：`git-ai upload-online-stats --base <baseCommit> --head <headCommit>`。
6. 建表 SQL 放在：`ai-cr-manage-service/docs/git_ai_online_stats.sql`。

第一阶段目标不是把所有看板功能做完，而是先回答：**这次上线总新增多少行，其中 AI、人、unknown 各多少，比例对不对。**

### 0.6 Phase 1 按顺序怎么做

1. 先在数据库执行 `ai-cr-manage-service/docs/git_ai_online_stats.sql`，至少建好 `git_ai_online_stats`。
2. 部署后端，确认 `/api/public/upload/ai-online-stats` 可访问。
3. 在 CI 里确定本次统计范围：`baseCommit` 和 `headCommit`。
4. CI 执行 `git-ai fetch-notes`，把远端 `refs/notes/ai` 拉下来。
5. CI 执行 `git-ai upload-online-stats --base <baseCommit> --head <headCommit>`。
6. 上传成功后，用 `/git-ai/dashboard/online/detail` 看最近明细。
7. 再用 `/git-ai/dashboard/online/stats` 看聚合总量。
8. 人工抽 3 到 5 个 MR，对照最终 diff 核对总新增行、AI 行、unknown 行是否合理。
9. 总量确认稳定后，再做成员榜和文件明细。
...
```bash
git-ai upload-online-stats --base <baseCommit> --head <headCommit>
```

等价范围写法：

```bash
git-ai upload-online-stats <baseCommit>..<headCommit>
```
...
git-ai upload-online-stats \
  ...
  --environment prod \
  --metric-scope release-delta \
  --merge-commit "$CI_COMMIT_SHA" \
...
```bash
git-ai upload-online-stats <baseCommit>..<headCommit> --dry-run
```

## 0A. 白话解释和补充说明

如果只想快速明白这份方案在说什么，读完第 0 章就够了；这一章只是把容易误解的点再用更白的话解释一遍。
